...
export const getPosts=(req,res)=>{
    ...
    jwt.verify(token,"secretkey",(err,userInfo)=>{
        ...
        const q=user_id && user_id!=="undefined" ? 
         `SELECT p.*,u.user_id AS user_id,name,profile_pic FROM posts AS p JOIN users AS u ON (u.user_id=p.user_id) WHERE p.user_id=? ORDER BY p.created_at DESC`
        :`SELECT DISTINCT p.*, u.user_id, u.name, u.profile_pic
        FROM (
            SELECT posts_id FROM posts WHERE user_id = ?
            UNION
            SELECT p.posts_id FROM posts AS p
            JOIN relationships AS r ON p.user_id = r.followed_userid
            WHERE r.follower_userid = ?
        ) AS unique_posts
        JOIN posts AS p ON unique_posts.posts_id = p.posts_id
